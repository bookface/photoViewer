...
int Exif::processEXIFDir(const char *DirStart
 ...
 // (-1) catches illegal zero case as unsigned underflows to positive large.
    ...
    // If its bigger than 4 bytes, the dir entry contains an offset.
        ...
        // Bogus pointer offset and / or bytecount value
            if (OffsetVal+ByteCount > exifSize) continue;
...
//qDebug() << "fist offset: " << FirstOffset;
    ...
    // invalid offset for first Exif IFD value ;
        if (FirstOffset < 16 || int(FirstOffset) > itemlen-16)  return -1;
    }
